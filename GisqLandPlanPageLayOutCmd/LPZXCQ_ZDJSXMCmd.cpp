// LPZXCQ_ZDJSXMCmd.cpp : CLPZXCQ_ZDJSXMCmd 的实现

#include "stdafx.h"
#include "LPZXCQ_ZDJSXMCmd.h"
#include "LPPrintMapSetsDlg.h"

// CLPZXCQ_ZDJSXMCmd

// STDMETHODIMP CLPZXCQ_ZDJSXMCmd::get_Enabled(VARIANT_BOOL * bEnabled )
// {
// 	//获取"行政区"信息
...
STDMETHODIMP CLPZXCQ_ZDJSXMCmd::OnClick()
{
...
// 	if (m_bstrXZQCode == NULL)return S_OK;
	...
	if (spGHWorkspace != NULL && spLYWorkspace != NULL)
	{
		...
		CString strConfigFile = strAppPath + _T("\\data\\template\\土地利用中心城区重点建设项目规划图.xml");
		CString strSection(_T("ZXCQ_ZDJSXM"));
		...
		strArrDatasetName.Add(_T("MZZDJSXM"));
