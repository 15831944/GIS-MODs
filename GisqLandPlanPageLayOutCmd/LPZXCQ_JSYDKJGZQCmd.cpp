// LPZXCQ_JSYDKJGZQCmd.cpp : CLPZXCQ_JSYDKJGZQCmd 的实现

#include "stdafx.h"
#include "LPZXCQ_JSYDKJGZQCmd.h"
#include "LPPrintMapSetsDlg.h"

// CLPZXCQ_JSYDKJGZQCmd

// STDMETHODIMP CLPZXCQ_JSYDKJGZQCmd::get_Enabled(VARIANT_BOOL * bEnabled )
// {
// 	//获取"行政区"信息
...
STDMETHODIMP CLPZXCQ_JSYDKJGZQCmd::OnClick()
{
...
// 	if (m_bstrXZQCode == NULL)return S_OK;
	...
	if (spGHWorkspace != NULL && spLYWorkspace != NULL)
	{
		...
		CString strConfigFile = strAppPath + _T("\\data\\template\\土地利用中心城区建设用地空间管制区图.xml");
		CString strSection(_T("ZXCQ_JSYDKJGZQ"));
