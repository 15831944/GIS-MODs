// LPZXCQ_JSZHCmd.cpp : CLPZXCQ_JSZHCmd 的实现

#include "stdafx.h"
#include "LPZXCQ_JSZHCmd.h"
#include "LPPrintMapSetsDlg.h"

// CLPZXCQ_JSZHCmd

// STDMETHODIMP CLPZXCQ_JSZHCmd::get_Enabled(VARIANT_BOOL * bEnabled )
// {
// 	//获取"行政区"信息
...
STDMETHODIMP CLPZXCQ_JSZHCmd::OnClick()
{
...
// 	if (m_bstrXZQCode == NULL)return S_OK;
	...
	if (spGHWorkspace != NULL && spLYWorkspace != NULL)
	{
		...
		CString strConfigFile = strAppPath + _T("\\data\\template\\土地利用中心城区基数转换标识图.xml");
		CString strSection(_T("ZXCQ_JSZH"));
		...
		strArrDatasetName.Add(_T("JSZH"));
		strArrDatasetName.Add(_T("JQDLTB"));
