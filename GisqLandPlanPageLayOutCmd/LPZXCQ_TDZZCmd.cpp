// LPZXCQ_TDZZCmd.cpp : CLPZXCQ_TDZZCmd 的实现

#include "stdafx.h"
#include "LPZXCQ_TDZZCmd.h"
#include "LPPrintMapSetsDlg.h"

// CLPZXCQ_TDZZCmd

// STDMETHODIMP CLPZXCQ_TDZZCmd::get_Enabled(VARIANT_BOOL * bEnabled )
// {
// 	//获取"行政区"信息
...
STDMETHODIMP CLPZXCQ_TDZZCmd::OnClick()
{
...
// 	if (m_bstrXZQCode == NULL)return S_OK;
	...
	if (spGHWorkspace != NULL && spLYWorkspace != NULL)
	{
		...
		CString strConfigFile = strAppPath + _T("\\data\\template\\土地利用中心城区土地整治规划图.xml");
		CString strSection(_T("ZXCQ_TDZZ"));
