// LPJSYDGZQ_FQCmd.cpp : CLPJSYDGZQ_FQCmd 的实现

#include "stdafx.h"
#include "LPJSYDGZQ_FQCmd.h"
#include "LPPrintMapSetsDlg.h"

// CLPJSYDGZQ_FQCmd

// STDMETHODIMP CLPJSYDGZQ_FQCmd::get_Enabled(VARIANT_BOOL * bEnabled )
// {
// 	//获取"行政区"信息
...
STDMETHODIMP CLPJSYDGZQ_FQCmd::OnClick()
{
...
// 	}
	...
	if (spGHWorkspace != NULL /*&& spLYWorkspace != NULL*/)
	{
		...
		//CString strConfigFile = strAppPath + _T("\\data\\template\\土地利用建设用地空间管制区图.xml");
		CString strConfigFile = strAppPath + CONFIGPATH;
		CString strSection(_T("JSYDGZQ_FQ"));
