// LPZXCQ_ZTGHCmd.cpp : CLPZXCQ_ZTGHCmd 的实现

#include "stdafx.h"
#include "LPZXCQ_ZTGHCmd.h"
#include "LPPrintMapSetsDlg.h"

// CLPZXCQ_ZTGHCmd

// STDMETHODIMP CLPZXCQ_ZTGHCmd::get_Enabled(VARIANT_BOOL * bEnabled )
// {
// 	//获取"行政区"信息
...
STDMETHODIMP CLPZXCQ_ZTGHCmd::OnClick()
{
...
// 	if (m_bstrXZQCode == NULL)return S_OK;
	...
	if (spGHWorkspace != NULL && spLYWorkspace != NULL)
	{
		...
		CString strConfigFile = strAppPath + _T("\\data\\template\\土地利用中心城区总体规划图.xml");
		CString strSection(_T("ZXCQ_ZTGH"));
