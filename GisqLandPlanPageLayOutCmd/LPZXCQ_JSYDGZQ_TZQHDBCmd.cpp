// LPZXCQ_JSYDGZQ_TZQHDBCmd.cpp : CLPZXCQ_JSYDGZQ_TZQHDBCmd 的实现

#include "stdafx.h"
#include "LPZXCQ_JSYDGZQ_TZQHDBCmd.h"
#include "LPPrintMapSetsDlg.h"

// CLPZXCQ_JSYDGZQ_TZQHDBCmd

// STDMETHODIMP CLPZXCQ_JSYDGZQ_TZQHDBCmd::get_Enabled(VARIANT_BOOL * bEnabled )
// {
// 	//获取"行政区"信息
...
STDMETHODIMP CLPZXCQ_JSYDGZQ_TZQHDBCmd::OnClick()
{
...
// 	if (m_bstrXZQCode == NULL)return S_OK;
	...
	if (spGHWorkspace != NULL && spLYWorkspace != NULL)
	{
		...
		CString strConfigFile = strAppPath + _T("\\data\\template\\土地利用中心城区建设用地空间管制区调整前后对比图.xml");
		CString strSection(_T("ZXCQ_JSYDGZQ_TZQHDB"));
