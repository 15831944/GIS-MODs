// LPJSZHBSTCmd.cpp : CLPJSZHBSTCmd 的实现

#include "stdafx.h"
#include "LPJSZHBSTCmd.h"
#include "LPPrintMapSetsDlg.h"

// CLPJSZHBSTCmd

// STDMETHODIMP CLPJSZHBSTCmd::get_Enabled(VARIANT_BOOL * bEnabled )
// {
// 	//获取"行政区"信息
...
STDMETHODIMP CLPJSZHBSTCmd::OnClick()
{
...
// 	}
	...
	if (spGHWorkspace != NULL/* && spLYWorkspace != NULL*/)
	{
		...
		//CString strConfigFile = strAppPath + _T("\\data\\template\\土地利用基数转换标识图.xml");
		CString strConfigFile = strAppPath + CONFIGPATH;
		CString strSection(_T("JSZHBST"));
